...
class WC_Stock_Management_List_Table extends WP_List_Table {
		...
		switch( $column_name ) {
			...
			case 'title' :
				...
				if ( $product->is_type( 'variation' ) ) {
					...
					if ( $this->last_product_id !== $product->id ) {
			...
			case 'id' :
				if ( ! $product->is_type( 'variation' ) ) {
					$this->last_product_id = $product->id;
					return $product->id;
				}
			...
			break;
			case 'manage_stock' :
				if ( $product->is_type( 'variation' ) && $product->managing_stock() && ! $product->variation_has_stock ) {
			...
			case 'stock' :
				$this->index++;
				?>
				<input type="text" class="input-text wc_bulk_stock_quantity_value" tabindex="<?php echo $this->index; ?>" data-name="stock_quantity[<?php echo $product->is_type( 'variation' ) ? $product->variation_id : $product->id; ?>]" placeholder="<?php
					if ( $product->managing_stock() ) {
						if ( ! $product->is_type( 'variation' ) || $product->variation_has_stock ) {
							echo wc_stock_amount( $product->stock );
						} else {
				...
				<input type="hidden" class="input-text" data-name="current_stock_quantity[<?php echo $product->is_type( 'variation' ) ? $product->variation_id : $product->id; ?>]" value="<?php if ( ! $product->is_type( 'variation' ) || $product->variation_has_stock ) echo $product->stock; ?>" />
				<?php
	...
	public function prepare_items() {
		...
		if ( $post_type ) {
			$post_types = $post_type === 'product' ? 'product' : 'product_variation';
