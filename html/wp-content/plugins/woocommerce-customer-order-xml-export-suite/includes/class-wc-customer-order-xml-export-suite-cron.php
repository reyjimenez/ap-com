	...
	public function auto_export_orders() {
		...
		if ( ! empty( $order_ids ) ) {
			...
			try {
				...
				wc_customer_order_xml_export_suite()->get_export_handler_instance()->start_export( $order_ids, array(
					'type'       => 'orders',
	...
	public function auto_export_customers() {
		...
		if ( ! empty( $customers ) ) {
			...
			try {
				...
				wc_customer_order_xml_export_suite()->get_export_handler_instance()->start_export( $customers, array(
					'type'       => 'customers',
					'method'     => $export_method,
					'invocation' => 'auto',
				) );
