...
class WC_Customer_Order_XML_Export_Suite_Method_SFTP extends WC_Customer_Order_XML_Export_Suite_Method_File_Transfer {
	...
	public function perform_action( $file_path ) {
		...
		$remote_path = "ssh2.sftp://{$this->sftp_link}/{$this->path}";
		$remote_file = "{$remote_path}{$filename}";

		if ( ! is_readable( $remote_path ) ) {

			/* translators: Placeholders: %s - file path */
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Remote path %s is not readable.', 'woocommerce-customer-order-xml-export-suite' ), $remote_path ) );
		}
