 ...
 *
 * @package     WC-Customer-Order-XML-Export-Suite/Export-Methods/FTP-Implicit-SSL
 * @author      SkyVerge
 ...
 * @since 2.0.7-1
 */
class WC_Customer_Order_XML_Export_Suite_Method_FTP_Implicit_SSL extends WC_Customer_Order_XML_Export_Suite_Method_File_Transfer {
	 ...
	 */
	 public function __construct( $args ) {
		...
		if ( ! $this->curl_handle ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'Could not initialize cURL.', 'woocommerce-customer-order-xml-export-suite' ) );
		}
		 ...
		 * @since 1.1.0
		 * @param array $options
		 * @param \WC_Customer_Order_XML_Export_Suite_Method_FTP_Implicit_SSL instance
		 */
		$options = apply_filters( 'wc_customer_order_xml_export_suite_ftp_over_implicit_curl_options', $options, $this );
		...
		foreach ( $options as $option_name => $option_value ) {
			...
			if ( ! curl_setopt( $this->curl_handle, $option_name, $option_value ) ) {
				...
				throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not set cURL option: %s', 'woocommerce-customer-order-xml-export-suite' ), $option_name ) );
			}
		}
	}
	...
	public function perform_action( $file_path ) {
		...
		if ( ! curl_setopt( $this->curl_handle, CURLOPT_URL, $this->url . $filename ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not set cURL file name: %s', 'woocommerce-customer-order-xml-export-suite' ), $filename ) );
		}
		...
		if ( ! $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not open %s for reading.', 'woocommerce-customer-order-xml-export-suite' ), $file_path ) );
		}
		...
		if ( ! curl_setopt( $this->curl_handle, CURLOPT_INFILE, $stream ) ) {
			...
			/* translators: Placeholders: %s - name of file to be updloaded */
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not load file %s', 'woocommerce-customer-order-xml-export-suite' ), $filename ) );
		}
		...
		if ( ! curl_exec( $this->curl_handle ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not upload file. cURL Error: [%1$s] - %2$s', 'woocommerce-customer-order-xml-export-suite' ), curl_errno( $this->curl_handle ), curl_error( $this->curl_handle ) ) );
		}
