...
class WC_Customer_Order_XML_Export_Suite_Admin {
	...
	public function load_wc_styles_scripts( $screen_ids ) {
		...
		$screen_ids[] = 'woocommerce_page_wc_customer_order_xml_export_suite';
	...
	public function process_export_bulk_actions() {
		...
		if ( 'woocommerce_page_wc_customer_order_xml_export_suite' !== $screen->id ) {
			return;
		}
	...
	private function is_export_screen() {
		...
		return in_array( $screen->id, array(
			'woocommerce_page_wc_customer_order_xml_export_suite',
			'shop_order',
			'edit-shop_order',
		), true );
	}
