 ...
 *
 * @package     WC-Customer-Order-XML-Export-Suite/Handler
 * @author      SkyVerge
...
class WC_Customer_Order_XML_Export_Suite_Handler {
	...
	public function transfer_export( $export_id, $export_method = null ) {
		...
		$_export_method = wc_customer_order_xml_export_suite()->get_methods_instance()->get_export_method( $export_method, $export->type, (array) $export );
	...
	public function get_exports_dir() {
		...
		return $upload_dir['basedir'] . '/xml_exports';
	}
	...
	public function get_exports_url() {
		...
		return $uploads_dir['baseurl'] . '/xml_exports';
	}
	...
	public function start_export( $ids, $args = array() ) {
		...
		if ( ! $filename ) {
			throw new SV_WC_Plugin_Exception( esc_html__( "No filename given for export file, can't export", 'woocommerce-customer-order-xml-export-suite' ) );
		}
		...
		if ( false === $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not open the export file %s for writing', 'woocommerce-customer-order-xml-export-suite' ), $file_path ) );
		}

		$background_export = $this->get_background_export_handler();
		$header            = $this->get_generator( $args['type'] )->get_header();
	...
	public function export_item( $item, $export ) {
		...
		if ( empty( $export->file_path ) ) {
			wc_customer_order_xml_export_suite()->log( esc_html__( 'No export file path provided, cannot export item.', 'woocommerce-customer-order-xml-export-suite' ) );
			return;
		}
		...
		if ( false === $stream ) {
			wc_customer_order_xml_export_suite()->log( esc_html__( 'Could not open the export file for appending.', 'woocommerce-customer-order-xml-export-suite' ) );
			return;
		}

		$generator = $this->get_generator( $export->type );
	...
	public function finish_export( $export ) {
		...
		if ( 'auto' === $export->invocation ) {
			 ...
			 */
			do_action( 'wc_customer_order_xml_export_suite_' . $export->type . '_exported', $export->object_ids );
		}
		...
		$footer = $this->get_generator( $export->type )->get_footer();
		...
		// TODO: remove the following deprecated filter in next major release {IT 2016-08-03}

		// filter the final generated XML
		$generated_xml = file_get_contents( $export->file_path );

		/**
		 * Allow actors to change the generated XML, such as removing headers.
		 *
		 * In 2.0.0 moved here from WC_Customer_Order_XML_Export_Suite_Writer class,
		 * removed $xml_array & $orders params, added $this param.
		 *
		 * @since 1.0.0
		 * @param string $xml - generated XML file
		 * @param \WC_Customer_Order_XML_Export_Suite_Handler $this - generator class instance
		 */
		$xml = apply_filters( 'wc_customer_order_xml_export_suite_generated_xml', $generated_xml, $this );

		file_put_contents( $export->file_path, $xml );
	...
	public function add_export_finished_notice( $export ) {
		...
		if ( 'manual' === $export->invocation ) {
			...
			$message_id = 'wc_customer_order_xml_export_suite_finished_' . $export->id;

			// add notice for manually created exports
			if ( $export->created_by && ! wc_customer_order_xml_export_suite()->get_admin_notice_handler()->is_notice_dismissed( $message_id, $export->created_by ) ) {

				$export_notices = get_user_meta( $export->created_by, '_wc_customer_order_xml_export_suite_notices', true );
				...
				update_user_meta( $export->created_by, '_wc_customer_order_xml_export_suite_notices', $export_notices );
			}
