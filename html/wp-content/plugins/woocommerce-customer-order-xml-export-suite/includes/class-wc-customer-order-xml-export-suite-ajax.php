 ...
 *
 * @package     WC-Customer-Order-XML-Export-Suite/AJAX
 * @author      SkyVerge
...
class WC_Customer_Order_XML_Export_Suite_AJAX {
	...
	public function get_export_status() {
		...
		$export = wc_customer_order_xml_export_suite()->get_export_handler_instance()->get_export( $_GET['export_id'] );

		if ( ! $export ) {

			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Not Found', 'woocommerce-customer-order-xml-export-suite' ),
				/* translators: Placeholders: %s - export ID */
				'message' => sprintf( esc_html__( 'No export found with id %s. It may have been cancelled during export.', 'woocommerce-customer-order-xml-export-suite' ), $_GET['export_id'] ),
			) );
		}
		...
		if ( 'failed' === $export->status ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Failed', 'woocommerce-customer-order-xml-export-suite' ),
				'message' => esc_html__( 'Unfortunately, your export failed.', 'woocommerce-customer-order-xml-export-suite' ) . ' ' . $logs_message,
			) );
		}
		...
		if ( 'failed' === $export->transfer_status ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Transfer Failed', 'woocommerce-customer-order-xml-export-suite' ),
				/* translators: Placeholders: %1$s - via [method], example: "...but the transfer via Email failed.", %2$s - opening <a> tag, %3$s - closing </a> tag */
				'message' => sprintf( esc_html__( 'Export completed successfully, but the transfer %1$s failed. Exported file is available under %2$sExport List%3$s.', 'woocommerce-customer-order-xml-export-suite' ), $label, '<a href="' . admin_url( 'admin.php?page=wc_customer_order_xml_export_suite&tab=export_list' ) . '">', '</a>' ) . ' ' . $logs_message,
			) );
		}
