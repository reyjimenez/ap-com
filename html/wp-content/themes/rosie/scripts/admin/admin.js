jQuery(document).ready(function($) {	
	var icons = [], myIconPicker = jQuery('input.select_an_icon_field,input[name="ozy_rosie_meta_post[ozy_rosie_meta_post_thumbnail_color_group][0][ozy_rosie_meta_post_thumbnail_icon]"]').fontIconPicker({iconsPerPage:2000});
	
	$('div[id*="ozy_rosie_meta_post"]').css('overflow', 'inherit');
	...
	*/
	...
	$(document).on('click', '.edit-menu-item-edit-style', function() {		
		...
		if (get_params !== undefined && get_params !== null) {
			if(get_params.html_shortcode !== undefined) { $('#custom-menu-html-shortcode').val($.base64.decode(get_params.html_shortcode)); }
			...
			$('#custom-menu-html-shortcode').val('');$('#custom-menu-bg-image').val('');$('#custom-menu-bg-repeat').val('no-repeat');$('#custom-menu-bg-size').val('auto');$('#custom-menu-bg-position-x').val('right');$('#custom-menu-bg-position-y').val('bottom');$('#custom-menu-dropdown-width').val('auto');$('#custom-menu-bg-image-preview>img').attr('src', '');
		...
		tb_show('Mega Menu Style', '#TB_inline?height=815&max-height=815&width=750&inlineId=ozyMegaMenuStyleWindow', false);
		$('#TB_ajaxContent').css('height', '90%');
	});
	...
	$(document).on('click', '#custom-menu-bg-apply', function(){
		if(ozy_current_target_menu_style_box != null) {
			var pass_params =  '{';
			pass_params += '"html_shortcode":"'+ $.base64.encode($('#custom-menu-html-shortcode').val()) +'",';
