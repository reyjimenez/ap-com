...
$caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $css_animation = $style = $external_style = $border_color = $css = $img_animated = $img_infinite = $img_animation = $zoom_on_hover = $lightbox = '';
...
/*ozy*/
//see variable list top of the file
$css_class2 = '';
if($img_animated === 'yes') {
	$css_class2 .= ' animated';
	if($img_infinite === 'yes') {	
		$css_class2 .= ' animation';
	}
	if($img_animation !== '') {	
		$css_class2 .= ' ' . $img_animation;
	}
}
...
if($lightbox === 'yes') {
	$css_class2 .= ' single-image-fancybox';
}
/*ozy end*/
...
$class_to_filter = 'wpb_single_image wpb_content_element vc_align_' . esc_attr($css_class2) . ' ' . $alignment . ' ' . $this->getCSSAnimation( $css_animation );
