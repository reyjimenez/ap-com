<?php
	class myHelper{
		...
		var $non_google_fonts = array('Verdana, Geneva, sans-serif', 'Georgia, Times New Roman, Times, serif', 'Courier New, Courier, monospace', 'Arial, Helvetica, sans-serif', 'Tahoma, Geneva, sans-serif', 'Palatino Linotype, Book Antiqua, Palatino, serif', 'Trebuchet MS, Arial, Helvetica, sans-serif', 'Arial Black, Gadget, sans-serif', 'Times New Roman, Times, serif', 'Lucida Sans Unicode, Lucida Grande, sans-serif', 'MS Serif, New York', 'Lucida Console, Monaco, monospace', 'Comic Sans MS, cursive');
		...
		*/		
		function render_google_fonts() {
			...
			if(count($this->active_google_fonts) > 0) {
				printf("<link rel='stylesheet' href='//fonts.googleapis.com/css?family=%s". $extended_params . $extended_params_subsets ."' type='text/css' />\r\n", str_replace(' ', '+', implode($extended_params . "%7C", array_filter($this->active_google_fonts))));				
			}			
		...
		*/		
		function render_custom_fonts() {
			...
			foreach($this->active_custom_fonts as $fnt) {		
				echo "@font-face {\r\n";
				echo "font-family: '". $fnt["title"] ."';\r\n";
						echo "src: url('". $fnt["eot"] ."');\r\n";
						echo "src: url('". $fnt["eot"] ."?#iefix') format('embedded-opentype'),\r\n";
						echo "url('". $fnt["woff"] ."') format('woff'),\r\n";
						echo "url('". $fnt["ttf"] ."') format('truetype'),\r\n";
						echo "url('". $fnt["svg"] ."#". $fnt["id"] ."') format('svg');\r\n";
				echo "font-weight: ". $fnt["weight"] .";\r\n";
				echo "font-style: ". $fnt["style"] .";\r\n"; 
				echo "}\r\n";
			}
		...
		function get_custom_font($font_name) { //, $font_weight = 'normal', $font_style = 'normal'
			if($font_name) {
				...
				if(isset($font->ID)) {
					$font_grp = vp_metabox('ozy_rosie_meta_font.ozy_rosie_meta_font_group', null, $font->ID);
					if(is_array($font_grp) && count($font_grp) > 0) {						
						foreach($font_grp as $fnt) {
							$id = $fnt['ozy_rosie_meta_font_id'];
							if(!in_array($id, $this->active_custom_fonts)) {
								$fnt_arr = array();
								$fnt_arr['title'] 	= $font->post_title;
								$fnt_arr['id'] 		= $id;
								$fnt_arr['eot'] 	= $fnt['ozy_rosie_meta_font_eot'];
								$fnt_arr['woff'] 	= $fnt['ozy_rosie_meta_font_woff'];
								$fnt_arr['ttf'] 	= $fnt['ozy_rosie_meta_font_ttf'];
								$fnt_arr['svg'] 	= $fnt['ozy_rosie_meta_font_svg'];
								$fnt_arr['weight'] 	= $fnt['ozy_rosie_meta_font_weight'];//$font_weight;
								$fnt_arr['style'] 	= $fnt['ozy_rosie_meta_font_style'];//$font_style;
		...
		*/		
		...
		{
			$background_slider_image_arr = vp_metabox('ozy_rosie_meta_page.ozy_rosie_meta_page_background_group.0.ozy_rosie_meta_page_background_slider_group');
	...
	jQuery.supersized({
		...
		slides : [';
			...
			foreach($background_slider_image_arr as $img) {
				$script .= $virgul . "{image:'" . $img['ozy_rosie_meta_page_background_slider_image'] . "', title:'', url:''}";
				$virgul = ",";
			}
$script .= '],
		progress_bar : 0,
		...
		*/		
		function fullscreen_video_show($poster = '', $mp4 = '', $webm = '', $ogv = '')
		{
			...
			if('computer' != $ozy_data->device_type) {
				$this->set_footer_style( "body { background-image: url(" . $poster . "); background-repeat: repeat; background-size: cover; }\r\n" );
				return null;
			}
		...
		*/			
		function fullscreen_vimeo_video_show($poster = '', $video_id = '')
		{
			...
			if(trim($video_id) != '') {
				$script = "
				jQuery(function() {
					jQuery.okvideo({video: '". $video_id ."',volume: 50, hd: true, adproof: true, annotations: false});
				});\n\r\t";			
		...
		*/			
		function fullscreen_youtube_video_show($poster = '', $video_id = '')
		{
			...
			if(trim($video_id) != '') {
				...
				$script = "
				jQuery(document).ready(function() {
					jQuery('body').tubular({videoId:'" . trim($video_id) . "', mute:0, repeat:1, start:0, wrapperZIndex:'-2'});
				});\n\r\t";
		...
		* @target - String				
		*/			
		function social_networks( $site, $username, $title, $target = "_self" ) {	
			$link_to_profile = '';
			$i = '';			
			switch( $site ) {
				case 'blogger':
			...
			echo '<a href="' . $link_to_profile . '" target="' . $target . '" class="symbol-' . $site . '"><span class="tooltip symbol" title="' . $title . '">'. $i .'</span></a>';//' . $site . '
		}
			...
			if(ozy_get_option('social_use') == '1') {
				...
				if(is_array($ozy_social_icons) && count($ozy_social_icons)>0) {
					foreach($ozy_social_icons as $s) {
						...
						if($account) {
							$this->social_networks( $s, $account, $this->get_array_value_by_key($s, $ozy_available_social_media_arr), ozy_get_option('social_icon_target') );
						}
					}
				}
			}
		...
		*/
		function post_royal_slider($hq = false) {
			...
			if(count($image_ids_arr)>0){
				foreach ( $image_ids_arr as $id ) {
					...
					$image_o.='<img class="rsImg" src="' . $small_image[0] . '" alt="image desc" />' . PHP_EOL;
				}
				
				return '<div class="royalSlider postGallerySlider rsMinCW">' . PHP_EOL . $image_o . PHP_EOL .'</div>';
			}
			return '';
		}
		...
		* newer_older_post_navigation_post
		*
		* Generates Next - Previous Post links for portfolio posts
		*
		* @padding - Boolean
		*/		
		function newer_older_post_navigation_portfolio($padding=false) {
			?>
            <div class="newer-older<?php echo $padding ? ' newer-older-blog' : '' ?>">
                <?php 
                    previous_post_link('%link', __('&larr; Previous Project', 'vp_textdomain')); 
                    if((int)ozy_get_option('page_portfolio_list_page_id')>0) {
                        echo '<a href="'. get_permalink(ozy_get_option('page_portfolio_list_page_id')) .'">' . __('All Projects', 'vp_textdomain') . '</a>';
                    }
                    next_post_link('%link', __('Next Project &rarr;', 'vp_textdomain')) 
                ?>
            </div><!--.newer-older-->           
            <?php			
		}
